...
from models.gmvae_small import gmvae_small
import util

### GET OUR DATA

# ## MNIST DATA
# dat = util.get_mnist(data_dir='assets/')
# dat.train.x = dat.train.images
# dat.train.y = (dat.train.labels.argmax(1) > 4.5).astype(np.float32)[:,None]
# # dat.train.y = dat.train.labels
# dat.test.x = dat.test.images
# dat.test.y = (dat.test.labels.argmax(1) > 4.5).astype(np.float32)[:,None]
# # dat.test.y = dat.test.labels
# DX = dat.train.x.shape[1]
# DY = dat.train.y.shape[1]

# ## NUMERAI DATA
inputs, targets, eras = util.make_numerai_batch('assets/numerai/numerai_training_data.csv')
# # FULL NUMERAI DATA: do era holdout for validation
...
holdouts = rng.choice(np.unique(eras), 1 + 1, replace=False).tolist()
P_TRAIN = 0.9
...
# PUT DATA IN STANDARD DATASET FORMAT
array_rank = lambda npa: len(npa.shape)
dat = util.Bunch()
# train set
dat.train = util.Bunch()
...
model = gmvae_small(DX, DY, DZ_NORMAL, DZ_BERNOULLI, D_HID, P_DROP, BN)
...
def _test(sess, d0, temp0, is_training, i_step, split_name):
    ...
    acc /= nn
    loss0 /= nn
    print('%s (%d samples) label_loss step %d: %.3f' % (split_name, nn, i_step, loss0))
    print('%s (%d samples) accuracy step %d: %.3f\n' % (split_name, nn, i_step, acc))
